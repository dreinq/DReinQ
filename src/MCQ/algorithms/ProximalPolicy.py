...
class ProximalPolicy(Restorable):
    ...
    def _trainingLoop(self, x, optims, shdrs, evalStep):
        ...
        while True:
            ...
            batchAdvs = batchRew - batchValues
            ...
            batchAdvs = (batchAdvs - batchAdvs.mean()) / batchAdvs.std()

            # batchData, codebook, batchB, batchNegLogProbs, batchRew, batchValues = self._chooseReplayBufferWithProbability(batchData, codebook, batchB, batchNegLogProbs, batchRew, batchValues)

            trainDatas = Zip(batchData, batchB, batchNegLogProbs, batchRew, batchValues, batchAdvs)
            ...
            del batchData, batchB, batchNegLogProbs, batchRew, C, trainLoader, trainDatas, batchAdvs
    ...
    def _train(self, policyOptimizer, valueOptimizer, dataLoader, C, icm):
        ...
        for trainData, batchB, oldNegLogProb, reward, oldValue, adv in tqdm(dataLoader, leave=False, ncols=40, bar_format="{l_bar}{bar}| [" + str(self._step1) + "] Train step"):
            if self.env.DoNormalizationOnObs:
            ...
            # else:
            policyLoss = self._policyLoss(trainData, C, batchB, oldNegLogProb, reward, adv)
            # if icm:
    ...
    def _policyLoss(self, trainData, C, batchB, oldNegLogProb, reward, advantage):
